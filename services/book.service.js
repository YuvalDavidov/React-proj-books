...
function getEmptyBook(title = '', authors = '', publishedDate = 2000, pageCount = 200, thumbnail = "http://coding-academy.org/books-photos/20.jpg") {
    return {
        id: '',
        title,
        authors,
...
function _createBooks() {
    ...
    if (!books || !books.length) {
        ...
        // books.push(_createBook('the grinch', 10))
        // books.push(_createBook('game of thrones', 20))
        // books.push(_createBook('harry potter', 15))
        // books.push(_createBook('alice in wonderland', 30))
        utilService.saveToStorage(BOOKS_KEY, books)
    }
}

function _createBook(title, price) {
    let book = getEmptyBook(title)
    book.id = utilService.makeId()
    book.description = utilService.makeLorem()
    book.thumbnail = './my-img.jpg'
    book.listPrice = {
        amount: price,
        currencyCode: 'EUR',
        isOnSeale: false
